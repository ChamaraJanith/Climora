...
import {
  ...
  Globe, LogOut, Search, Plus, Trash2, Edit2, AlertTriangle,
  ChevronDown, X,
} from 'lucide-react';
  ...
  return (
    <header className="h-16 bg-white border-b border-gray-100 flex items-center justify-between px-6 sticky top-0 z-30">
      ...
      <div className="flex items-center gap-3">
        <button className="relative w-9 h-9 rounded-xl bg-gray-50 border border-gray-200 flex items-center justify-center text-gray-500 hover:text-[#06b6d4] hover:border-[#06b6d4] transition-colors">
          <Bell size={18} />
        </button>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}
        ...
        <input type="number" min="1" value={amount} onChange={e => setAmount(Number(e.target.value))}
          className="w-full px-3 py-2.5 rounded-xl border border-gray-200 text-sm text-gray-800 bg-white focus:outline-none focus:ring-2 focus:ring-[#06b6d4]/30 focus:border-[#06b6d4] transition mb-4" />
...
export default function ReliefItemsPage() {
  ...
  const [adjustTarget, setAdjustTarget] = useState(null); // { item, shelterId }
  ...
  // Flatten all items across shelters
  ...
  const filtered = allItems.filter(item => {
    ...
    const matchSearch = !q || item.name?.toLowerCase().includes(q) || item.shelterName?.toLowerCase().includes(q);
      ...
      <div className="flex-1 ml-64 flex flex-col min-h-screen">
        ...
        <main className="flex-1 p-6 space-y-6 bg-white">
          ...
          <div className="bg-[#F9FAFB] rounded-2xl border border-gray-100 p-4 flex flex-wrap items-center gap-3">
            ...
            {filterCategory !== 'all' && (
              <button onClick={() => setFilterCategory('all')} className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-medium text-gray-500 hover:text-red-500 border border-gray-200 bg-white transition">
                <X size={13} /> Clear
              </button>
            )}
          </div>
