  ...
  return (
    <div className="max-w-7xl mx-auto relative">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        {/* ==================================================== */}
        <div className="lg:col-span-2 bg-[linear-gradient(135deg,#020617,#0f172a)] border border-white/10 backdrop-blur-xl rounded-2xl shadow-[0_20px_60px_rgba(0,0,0,0.6)] p-6 space-y-6">
          {/* Main Info Card */}
          <div>
            ...
            <div className="flex flex-wrap items-center gap-4 text-sm text-white/40 font-medium pb-4 border-b border-white/10">
            ...
            <div className="mt-4">
              <h3 className="text-base font-bold text-white mb-2 tracking-tight">Incident Description</h3>
          ...
          {/* Photos Box */}
          <div>
            <h3 className="text-base font-bold text-white mb-3 tracking-tight">Attached Imagery</h3>
            {report.photos && report.photos.length > 0 ? (
              <div className="grid grid-cols-2 gap-4">
                {report.photos.map((url, i) => (
                  <button 
                    ...
                    className="aspect-video lg:aspect-square w-full rounded-xl overflow-hidden border border-white/10 hover:opacity-90 hover:ring-2 hover:ring-cyan-500 transition-all cursor-zoom-in"
                  >
              ...
              <div className="w-full py-12 bg-[#020617]/60 border border-dashed border-white/10 rounded-xl flex flex-col items-center justify-center text-white/40">
                <Icons.Image />
                <span className="text-sm font-medium mt-2 text-white/70">No visual evidence provided</span>
              </div>
            )}
          </div>
        </div>
        ...
        {/* ==================================================== */}
        <div className="lg:col-span-1 bg-[linear-gradient(135deg,#020617,#0f172a)] border border-white/10 backdrop-blur-xl rounded-2xl shadow-[0_20px_60px_rgba(0,0,0,0.6)] p-6 space-y-6">
          <div>
            <h3 className="text-base font-bold text-white mb-3 tracking-tight">Geographic Location</h3>
            ...
            {report.location?.lat && report.location?.lon ? (
              ...
              <div className="w-full h-64 bg-[#020617]/60 border border-dashed border-white/10 rounded-xl flex items-center justify-center text-white/40">
                <span className="text-sm font-medium">Map data unavailable</span>
              </div>
            )}
            
            <div className="mt-4 text-xs text-white/40 bg-white/5 p-3 rounded-lg border border-white/10 font-medium">
              Georeferenced to: <span className="font-bold text-white/70">{report.location?.district}</span>
              <br /><span className="text-white/40 font-normal">LAT: {report.location?.lat?.toFixed(5)} / LON: {report.location?.lon?.toFixed(5)}</span>
            </div>
          </div>
          ...
          {report.weatherContext && report.weatherContext.summary && (
            ...
            <div>
              <div className="flex items-start gap-3">
                <div className="bg-cyan-500/20 text-cyan-300 p-2.5 rounded-xl shrink-0 shadow-sm">
                ...
                <div className="min-w-0">
                  <h4 className="text-sm font-bold text-white/40 mb-1 tracking-tight">Weather Context</h4>
                  <p className="text-sm text-white/80 font-medium">{report.weatherContext.summary}</p>
